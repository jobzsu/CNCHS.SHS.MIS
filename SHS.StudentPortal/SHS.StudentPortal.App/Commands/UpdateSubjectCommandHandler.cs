...
internal sealed class UpdateSubjectCommandHandler : ICommandHandler<UpdateSubjectCommand>
{
    ...
    public async Task<ResultModel> Handle(UpdateSubjectCommand request, CancellationToken cancellationToken)
    {
        ErrorModel error;
        ...
        {
            _logger.LogError("Error while updating subject: {ex}", ex);

            error = new(nameof(Exception), "Error while updating Subject");
